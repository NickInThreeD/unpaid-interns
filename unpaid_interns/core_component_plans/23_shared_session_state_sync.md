...
- Currently: `CurrentDay`, `DaysUntilDeadline`, `TeamCredits`, `CurrentQuota`, `QuotaProgress`, `QuotasCompleted`, `RunState`, `RoundPhase`, `RoundStartTick`, `DepartureStartTick` ([`105_departure_and_extraction_resolution.md`](105_departure_and_extraction_resolution.md)), `SelectedLocationId`, `WeatherId` ([`35_environmental_conditions_weather.md`](35_environmental_conditions_weather.md)), `RunSeed` and `RoundSeed`, crew roster entries, power zone flags ([`36_lighting_and_power_grid.md`](36_lighting_and_power_grid.md)), and the banked total.
- `DepartureStartTick` is the sharpest case for this file's whole argument. It is replicated as absolute state so the remaining window is *derived* rather than counted down locally — a client counting its own timer will disagree with the server about whether an intern made it, and that disagreement decides whether someone lives. It is also the one value where the RPC and the state must both be right: the announcement has to arrive instantly, and a client that missed it must still show the correct countdown.
