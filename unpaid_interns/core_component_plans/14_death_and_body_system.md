...
- Depositing the body in the extraction zone counts as recovery, using that zone's inside test rather than a second one ([`43_loot_banking_deposit.md`](43_loot_banking_deposit.md)). Leaving the location without it does not. Recovery is decided at the point of no return along with everything else ([`105_departure_and_extraction_resolution.md`](105_departure_and_extraction_resolution.md)), which means a body dragged in during the departure window still counts — that last-second rescue is one of the best things this system produces and it must not be cut off early.
- **An intern left behind is a fourth case**, not a variant of death: they were alive when the round ended and simply did not make it back. If the recommended rule in [`105_departure_and_extraction_resolution.md`](105_departure_and_extraction_resolution.md) is adopted — lost with the location — then a body exists in principle and can never be recovered, so the penalty is the unrecovered one and the report says *missing* rather than *deceased*. Whichever rule is chosen, this component and that one must describe the same outcome.
