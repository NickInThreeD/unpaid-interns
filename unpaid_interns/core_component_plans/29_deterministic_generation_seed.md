...
- **One thing downstream of generation is deliberately exempt: NavMesh baking.** It runs on the server only and no client ever compares its result to anyone else's ([`30_runtime_navmesh_baking.md`](30_runtime_navmesh_baking.md)), so it needs no stream and no determinism guarantee. Saying so explicitly prevents someone spending a week making a baker reproducible for no benefit.
