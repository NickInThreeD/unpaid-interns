...
- **Do not make monsters scannable.** A scanner that reveals threats replaces the audio-perception skill the whole threat layer is built around. If a monster-detection tool is wanted, it belongs in §5 as a purchasable, limited, noisy item with a real cost ([`44_tool_and_equipment_items.md`](44_tool_and_equipment_items.md)).
- The related-but-different cases are both allowed and both worth having: **vents are scannable** ([`52_spawn_points_and_vents.md`](52_spawn_points_and_vents.md)) and **hazards are scannable** ([`59_static_map_hazards.md`](59_static_map_hazards.md)). Neither reveals a live threat — they reveal where threats *can come from* and where the floor is dangerous, which is route information the crew can act on. Revealing the vent is not revealing the thing in it, and holding that line is what keeps the scanner a navigation tool rather than a radar.
