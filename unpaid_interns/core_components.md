...
- ❌ **Deterministic Generation Seed [MVP]** — The server rolls one seed per round and replicates it; every client generates the identical layout. Without this, clients disagree about geometry. A `FixedRandom` singleton already exists in `ServerGameSystem.OnCreate` and is the natural place to source it.
...
- ❌ **Physics Props & Throwing** — Dropped items behaving as physics bodies that can be thrown, dropped down stairwells, or used to trigger hazards from a distance. `com.unity.physics` (DOTS physics) is installed and used for character collision, but no item physics exists. Determines whether "throw the noisy item to distract the monster" is possible at all.
